...
| 文件                                      | 改动                             |
| ----------------------------------------- | -------------------------------- |
| `src/features/collage-editor/lib/file.ts` | 上传时压缩图片，返回 Blob URL    |
| `src/features/collage-editor/types.ts`    | GalleryImage 扩展 blobUrl 等字段 |
...
### 5. `src/features/collage-editor/Editor.tsx:201,206` ✅ 已修复
...
### 6. `src/features/collage-editor/components/PhotoGallery/PhotoGallery.tsx:79,426` ✅ 已修复
...
- `src/features/collage-editor/types.ts` - GalleryImage 扩展 blobUrl 等字段
- `src/features/collage-editor/lib/file.ts` - 上传时自动压缩图片
- `src/features/export/lib/exporters.ts` - 新增 downloadAlbumPDF 函数
...
1. **压缩参数调优**：A4 纸放 12 张图，单图尺寸约 185×164 pt，可尝试更低质量（如 0.7-0.8）
...
await downloadAlbumPDF(
  ...
  (progress) => {
    console.log(
      `${Math.round(progress.progress * 100)}% - ${progress.message}`
    );
  }
);
```

每页布局：4 列 × 3 行 = 12 张图，A4 横向。
