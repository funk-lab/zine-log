import { GradientOptions, Pattern } from "fabric";
...
const AngleCoordsMap: Record<number, string> = {
...
const transformCoords2Angel = (coords: any) => {
  ...
  for (const key of keys) {
    ...
    if (JSON.stringify(_coords) === AngleCoordsMap[Number(key)]) {
      return Number(key);
    }
  }
  return 90;
...
export const transformFill2Colors = (v: any) => {
  if (!v || typeof v === "string" || v instanceof Pattern) {
...
export const transformColors2Fill = (
  v: any
): string | GradientOptions<"linear" | "radial"> | undefined => {
  let fill: string | GradientOptions<"linear" | "radial"> | undefined;
  switch (v?.type) {
    case "solid":
      fill = v.color;
      break;
